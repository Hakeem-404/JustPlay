...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_class c
    JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE c.relname = 'message_reactions' 
    AND n.nspname = 'public'
    AND c.relrowsecurity = true
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_class c
    JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE c.relname = 'message_read_status' 
    AND n.nspname = 'public'
    AND c.relrowsecurity = true
  ) THEN
...
DO $$
BEGIN
  -- Drop existing policies for game_messages
  DROP POLICY IF EXISTS "Game participants can view messages" ON game_messages;
  DROP POLICY IF EXISTS "Game participants can send messages" ON game_messages;
  DROP POLICY IF EXISTS "Users can update their own messages" ON game_messages;
  DROP POLICY IF EXISTS "Game organizers can delete any message" ON game_messages;
  
  -- Drop existing policies for message_reactions
  DROP POLICY IF EXISTS "Game participants can view reactions" ON message_reactions;
  DROP POLICY IF EXISTS "Game participants can add reactions" ON message_reactions;

/*
  # Game Participants System with Organizer Auto-Join
  ...
  2. Security
    ...
    - Add policies for viewing, joining, and updating participation

  3. Functions
    - `join_game()` - Handle joining games with waitlist support
    - `leave_game()` - Handle leaving games with waitlist promotion
    - `get_game_participants()` - Get participant list for a game

  4. Triggers
    - Auto-update `updated_at` timestamp

  5. Updated Logic
    - Organizer is automatically counted in participant totals
    - Game starts with current_players = 1 (organizer)
    - Capacity checking includes organizer in total count
*/
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE c.relname = 'game_participants' 
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Users can view participants of games they can see" ON game_participants;
  ...
  DROP POLICY IF EXISTS "Public profiles are viewable by authenticated users" ON game_participants;
  DROP POLICY IF EXISTS "Users can read own profile" ON game_participants;
  DROP POLICY IF EXISTS "Users can insert own profile" ON game_participants;
END $$;
...
-- Function to join a game (updated for organizer-counted system)
...
BEGIN
  ...
  -- Count current active participants (including organizer)
  ...
  -- Game is full when total participants (including organizer) >= max_players
  IF current_participants_count < game_record.max_players THEN
    ...
    -- Update game current_players count (total participants including organizer)
    UPDATE games 
...
-- Function to leave a game (updated for organizer-counted system)
...
BEGIN
  ...
  IF game_record.organizer_id = auth.uid() THEN
    RAISE NOTICE 'Organizer cannot leave their own game';
    RETURN json_build_object('success', false, 'error', 'Cannot leave your own game');
  END IF;
  ...
  IF user_participation.status = 'joined' THEN
    ...
    IF FOUND THEN
      ...
      -- Count remains the same since we're replacing one joined user with another
    ELSE
      -- Decrease current_players count (but keep minimum of 1 for organizer)
      UPDATE games 
...
-- Function to get game participants (updated for organizer-counted system)
